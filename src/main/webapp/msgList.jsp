	...
	<div class="content">
		<c:choose>
			<c:when test="${empty employee}">
				<h2 style="color: lightblue;text-align: center;margin-top: 20px;">您还没有进行身份识别，无法查看消息列表</h2>
			</c:when>
			<c:otherwise>
				<dl>
				...
				<div align="center" style="border: 0px; margin-bottom: 20px; margin-top: 20px;">
					<c:choose>
						<c:when test="${page.prePage}">
							<a href="GetMsgListServlet?currentPage=1">首页</a>|
							<a href="GetMsgListServlet?currentPage=${page.currentPage-1}">上一页</a>
						</c:when>
						<c:otherwise>
							首页|上一页
						</c:otherwise>
					...
					<c:choose>
						<c:when test="${page.nextPage}">
							<a href="GetMsgListServlet?currentPage=${page.currentPage+1}">下一页</a>|
							<a href="GetMsgListServlet?currentPage=${page.totalPage}">最后一页</a>
						</c:when>
						<c:otherwise>
							下一页|最后一页
						</c:otherwise>
